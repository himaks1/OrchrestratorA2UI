...
for port in 10011 10012 10013 10014; do
  echo "Waiting for subagent on port $port..."
  while ! python -c "import urllib.request; urllib.request.urlopen('http://localhost:$port/.well-known/agent-card.json')" 2>/dev/null; do
    sleep 1
  done
done
