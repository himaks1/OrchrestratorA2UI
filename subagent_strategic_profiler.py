...
@click.option("--port", default=10020, type=int)
def main(host, port):
    lite_llm_model = os.getenv("LITELLM_MODEL", "gemini/gemini-3.5-flash")
    agent = LlmAgent(
        name="subagent_strategic_profiler",
        description="Extracts and analyzes strategic metadata: SWOT analysis, key leadership, digital priorities, company direction, challenges, and solution opportunity assessments. It cross-references this with client profiles (digital strategic priorities, main products/services) to synthesize actionable strategic insights.",
        instruction="""You are the Strategic Profiler sub-agent for Ollie Sales Assistant.
Your primary role is to evaluate corporate SWOT, strategic priorities, spend outlook, and risk insights.
Use your database tools to query `company_financials` metadata and `clients`.
...
1. `company_financials`: Contains fields like id, company_name, ticker, industry, industry_code, sub_industry, hq_location, ceo_name, employee_count, ownership_type, parent_company, parent_country, site_count, core_products, export_pct, estimated_revenue, revenue_currency, fiscal_year, corporate_revenue, net_income, gross_margin_pct, operating_margin_pct, net_margin_pct, segment_breakdown (JSONB), competitor_data (JSONB), metadata (JSONB field containing SWOT, key leadership/CEO details, company direction, challenges, and solution opportunity assessments), ai_provenance, created_at, and updated_at.
2. `clients`: Contains fields like id, company_name, ownership_type, industry_sector, main_products_services, key_subsidiaries, number_of_employees, number_of_branches, ebitda_margin_pct, profit_margin_pct, digital_strategic_priorities (text/JSONB list of digital priorities), group_name, division, department, sales_person, created_at, and updated_at.

**Database Query Rules:**
- **Fuzzy Company Name Matching:** When filtering by a company or client name, always use case-insensitive fuzzy matching (e.g., `company_name ILIKE '%TargetName%'`) to account for variations in suffixes like "Inc.", "LLC", or "Corp".
- **JSONB Querying:** The `metadata`, `segment_breakdown`, and `competitor_data` fields are stored as `JSONB`. To extract values from them, you MUST use native PostgreSQL JSONB operators (such as `->` to get a JSON object, `->>` to get text, or `jsonb_array_elements()` to expand arrays) rather than standard string matching. E.g., `metadata->>'SWOT'` or `metadata->>'challenges'`.
...
Here is the strategic profile report:
...
""",
    ...
    skills = [
        AgentSkill(
            id="strategic_profiler_analysis",
            name="Strategic Profiler",
            description="Extracts and analyzes strategic metadata including SWOT, corporate challenges, digital transformation priorities, leadership details, and opportunity pitches.",
            tags=["strategy", "swot", "leadership", "priorities", "spend_outlook", "ebitda"],
            examples=[
                "What is the SWOT (Strengths, Weaknesses, Opportunities, Threats) analysis profile for [Company Name]?",
                "What are the primary operational challenges, market headwinds, or risk insights facing [Company Name]?",
                "What are the key digital strategic priorities and digital transformation focus areas for [Company Name]?",
                "Who are the key executive leaders (e.g., CEO name and details) at [Company Name]?",
                "What is the solution opportunity assessment and corresponding confidence score for pitching B2B solutions to [Company Name]?",
                "What are the main products/services offered by [Company Name], and what are its key subsidiaries?",
                "What is [Company Name]'s overall B2B spend outlook and commercial direction for the upcoming fiscal years?"
            ],
        )
    ]
    ...
    agent_card = AgentCard(
        name="subagent_strategic_profiler",
        description="Extracts and analyzes strategic metadata: SWOT, key leadership, digital priorities, company direction, challenges, and solution opportunity assessments.",
        url=base_url,
        version="1.0.0",
    ...
    logger = logging.getLogger("subagent_strategic_profiler")
    logger.info(f"Starting subagent_strategic_profiler on {host}:{port}")
